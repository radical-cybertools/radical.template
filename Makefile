...
ifndef NAME
	...
	echo "UNAME: $(UNAME)"
	echo rm -rf .git
	find . -name '*###cname###*' | sort  -u |    sed -e "p;s/###cname###/$(CNAME)/g" | xargs -r -n2 mv -v
	find . -name '*###lname###*' | sort  -u |    sed -e "p;s/###lname###/$(LNAME)/g" | xargs -r -n2 mv -v
	find . -name '*###uname###*' | sort  -u |    sed -e "p;s/###uname###/$(UNAME)/g" | xargs -r -n2 mv -v
